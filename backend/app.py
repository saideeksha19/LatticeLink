...
# ============================================================
# SOCKET.IO
# IMPORTANT: threading mode
# Eventlet has been removed because Render was running Python 3.14
# and Eventlet was causing the deployment crash.
# ============================================================
...
def auto_migrate_schema():
    """
    Safely inspect and migrate SQLite columns
    ...
    db_path = Config.SQLALCHEMY_DATABASE_URI.replace(
        "sqlite:///",
        ""
    )
    ...
    migrations = [
        ...
        (
            "keys",
            "key_type",
            "VARCHAR(50) DEFAULT 'ML-KEM-768/ML-DSA-65'"
        ),
    ...
    for table, column, col_type in migrations:
        ...
        try:
            ...
            if existing_cols and column not in existing_cols:
                ...
                logger.info(
                    f"[AutoMigrate] Added missing column "
                    f"{column} to {table}"
                )
            ...
            logger.warning(
                f"[AutoMigrate Warning] "
                f"Could not alter {table}.{column}: {e}"
            )
...
with app.app_context():
    ...
    try:
        ...
        logger.info(
            "Database and uploads directory initialized "
            "successfully."
        )
...
def health():
    ...
    try:
        ...
        logger.error(
            f"Health check database failure: {e}"
        )
