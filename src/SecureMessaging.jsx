...
import { 
  ...
  UserPlus, LogOut, X, CheckCheck, PhoneOff, Camera, Lock
} from 'lucide-react';
...
const SecureMessaging = () => {
  ...
  // --- Chat & Multi-User State ---
  const [chats, setChats] = useState([]);
  const [selectedChatId, setSelectedChatId] = useState('');

  const [allMessages, setAllMessages] = useState(() => {
    const saved = localStorage.getItem('ll_messages_v2');
  ...
  useEffect(() => {
    if (currentUser) {
      ...
      if (savedContacts) {
        ...
        setChats(parsed);
        if (parsed.length > 0 && !selectedChatId) setSelectedChatId(parsed[0].id);
      } else {
        setChats([]);
        setSelectedChatId('');
      }
  ...
  useEffect(() => {
    if (currentUser) {
      localStorage.setItem(`ll_contacts_${currentUser}`, JSON.stringify(chats));
    }
  }, [chats, currentUser]);

  // Save all messages globally
  useEffect(() => {
    localStorage.setItem('ll_messages_v2', JSON.stringify(allMessages));
  }, [allMessages]);
  ...
  const activeConvId = selectedChatId ? getConversationId(currentUser, selectedChatId) : null;
  const activeMessages = activeConvId && allMessages[activeConvId] ? allMessages[activeConvId] : [];
  ...
  const appendMessage = (msgData) => {
    ...
    setTimeout(() => {
      setAllMessages(prev => {
        ...
        return {
          ...prev,
          [activeConvId]: msgs.map(m => m.id === tempId ? { ...m, status: 'sent' } : m)
        };
      });
    }, 800);
  };
  ...
  const handleConfirmAddContact = (e) => {
    e.preventDefault();
    const otherUser = newContactName.trim();
    
    if (otherUser && otherUser !== currentUser) {
      // Add to MY contacts
      if (!chats.find(c => c.id === otherUser)) {
        setChats([...chats, { id: otherUser, name: otherUser, status: 'Offline' }]);
      }
      
      // Implicitly add ME to THEIR contacts so they see the chat
      const theirContactsStr = localStorage.getItem(`ll_contacts_${otherUser}`);
      let theirContacts = theirContactsStr ? JSON.parse(theirContactsStr) : [];
      if (!theirContacts.find(c => c.id === currentUser)) {
        theirContacts.push({ id: currentUser, name: currentUser, status: 'Offline' });
        localStorage.setItem(`ll_contacts_${otherUser}`, JSON.stringify(theirContacts));
      }

      setSelectedChatId(otherUser);
  ...
  const handleFileChange = (e) => {
    ...
    if (file && file.type.startsWith('image/')) {
      ...
      alert('Only image attachments are supported in this prototype.');
    }
  };
    ...
    return (
      <div className='dashboard-container' style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
        ...
        <div className="glass-card" style={{ padding: '40px', width: '100%', maxWidth: '400px', zIndex: 10, textAlign: 'center' }}>
          ...
          <p style={{ color: '#94a3b8', fontSize: '14px', marginBottom: '32px' }}>Enter your operator alias to access secure messaging.</p>
    ...
    <div className='dashboard-container' style={{ minHeight: '100vh', display: 'flex', position: 'relative', overflow: 'hidden' }}>
      ...
      {isAddContactOpen && (
        <div style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.7)', zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', backdropFilter: 'blur(4px)' }}>
          <div className="glass-card" style={{ padding: '32px', width: '100%', maxWidth: '350px', position: 'relative' }}>
            ...
            <h3 style={{ color: 'white', margin: '0 0 8px 0', fontSize: '18px' }}>Add New Operator</h3>
            <p style={{ color: '#64748b', fontSize: '13px', marginBottom: '24px' }}>Enter the exact alias of the operator to establish a secure link.</p>
            
            <form onSubmit={handleConfirmAddContact} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              <input 
                type="text" 
              ...
              <div style={{ display: 'flex', gap: '12px' }}>
                <button type="button" onClick={() => setIsAddContactOpen(false)} style={{ flex: 1, padding: '10px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', color: 'white', borderRadius: '8px', cursor: 'pointer' }}>
                  Cancel
                </button>
      ...
      {activeCall && (
        <div style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.85)', zIndex: 100, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', backdropFilter: 'blur(8px)' }}>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '24px' }}>
            <div style={{ 
              width: '120px', height: '120px', borderRadius: '50%', background: 'rgba(59, 130, 246, 0.2)', border: '2px solid #3b82f6', 
              display: 'flex', alignItems: 'center', justifyContent: 'center', 
              animation: 'pulse 1.5s infinite',
              boxShadow: '0 0 30px rgba(59, 130, 246, 0.5)'
            }}>
              {activeCall === 'video' ? <Camera size={48} color="#3b82f6" /> : <Phone size={48} color="#3b82f6" />}
            </div>
            
            <div style={{ textAlign: 'center' }}>
              <h2 style={{ color: 'white', fontSize: '32px', margin: '0 0 8px 0' }}>{activeChat?.name}</h2>
              <p style={{ color: '#10b981', fontSize: '18px', margin: 0 }}>Establishing Secure {activeCall === 'video' ? 'Video' : 'Audio'} Link...</p>
            </div>
      ...
      <div className="glass-card" style={{ width: '300px', borderRight: '1px solid rgba(59, 130, 246, 0.2)', display: 'flex', flexDirection: 'column', zIndex: 10 }}>
        <div style={{ padding: '24px', borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
            <h2 style={{ color: 'white', margin: 0, fontSize: '20px' }}>Messages</h2>
            <button onClick={openAddContactModal} style={{ background: 'transparent', border: 'none', cursor: 'pointer', color: '#3b82f6', display: 'flex', alignItems: 'center', gap: '4px' }} title="Add Contact">
              <UserPlus size={18} />
            </button>
          </div>
          ...
          <div className="input-group" style={{ padding: '8px 12px', marginBottom: '16px' }}>
            ...
            <input type="text" placeholder="Search chats..." style={{ fontSize: '14px' }} />
          </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto' }}>
          {chats.length === 0 ? (
            <div style={{ color: '#64748b', fontSize: '13px', textAlign: 'center', padding: '32px 16px' }}>
              No active secure links. Click the + icon to add a contact.
            </div>
          ) : (
            chats.map((chat) => (
              <div 
                ...
                style={{ 
                  ...
                  background: selectedChatId === chat.id ? 'rgba(59, 130, 246, 0.1)' : 'transparent',
                  borderLeft: selectedChatId === chat.id ? '3px solid #3b82f6' : '3px solid transparent',
                  cursor: 'pointer'
                }}
              >
                <div style={{ color: 'white', fontWeight: selectedChatId === chat.id ? '600' : '400' }}>{chat.name}</div>
        ...
        {activeChat ? (
          <>
            ...
            <div className="glass-card" style={{ padding: '20px 32px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid rgba(59, 130, 246, 0.2)', borderRadius: 0 }}>
              <div>
                <h2 style={{ color: 'white', margin: 0, fontSize: '20px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                  {activeChat.name} <ShieldCheck size={16} color="#10b981" />
                </h2>
                <span style={{ color: '#10b981', fontSize: '12px' }}>Quantum Secure Session Active</span>
              </div>
            ...
            <div style={{ background: 'rgba(245, 158, 11, 0.1)', border: '1px solid rgba(245, 158, 11, 0.3)', padding: '12px', borderRadius: '8px', margin: '24px 32px 0 32px', display: 'flex', alignItems: 'center', gap: '8px', color: '#fcd34d', fontSize: '13px', justifyContent: 'center' }}>
              <Lock size={14} /> Messages to this chat are secured with ML-KEM and AES-256-GCM. No one outside this chat can read them.
            </div>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '32px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
              {activeMessages.length > 0 ? (
                ...
                <div style={{ color: '#64748b', fontSize: '14px', textAlign: 'center', marginTop: '40px' }}>
                  No messages yet. Send a message to open the secure tunnel.
                </div>
              )}
            </div>
            ...
            <div className="glass-card" style={{ padding: '20px 32px', borderTop: '1px solid rgba(59, 130, 246, 0.2)', borderRadius: 0 }}>
              <form onSubmit={handleSend} style={{ display: 'flex', gap: '16px', alignItems: 'center' }}>
                ...
                <input 
                  ...
                  placeholder={`Type a secure message to ${activeChat.name}...`} 
          ...
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', color: '#64748b' }}>
            <ShieldCheck size={64} color="#1e293b" style={{ marginBottom: '16px' }} />
            <h2>No Secure Link Selected</h2>
            <p>Select a contact from the sidebar or add a new one to start messaging.</p>
          </div>
        )}
      </div>
    </div>
